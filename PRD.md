...
3. **Long-horizon overnight execution from GitHub.** The executor runs on a different machine and treats GitHub as the only canonical input. The operator is asleep. The agent does not interim-report, does not ask for routine permission, and does not engage the user until it has completed the PRD end to end, hit a genuine boundary/credential blocker that prevents all useful work, or exhausted all Mac/local-feasible work and reached the first hard increased-compute dependency. The agent decides every contested call autonomously using the discipline in this PRD. The agent updates GitHub continuously and Hugging Face for bulk artifacts. The operator reviews from a different machine.
...
If the agent reaches the point where all useful Mac/local work is exhausted and increased compute is the next hard dependency, it writes a compute-escalation section with exact hardware, storage, credential, command, and artifact requirements; commits and pushes the current state; then advises the operator. That report point is allowed only after local execution has been maximised.
...
If a stuck point is unresolvable, the agent logs it in `EXECUTION-STATE.md` with `BLOCKED:<reason>:<workaround-attempted>`, continues with the workaround, and moves on. The agent does not stop and ask. The operator reviews blockers on review. The only acceptable early stop is a boundary violation, missing credentials that prevent all useful local work, a destructive ambiguity that cannot be safely stubbed, or a compute-escalation boundary reached after all useful Mac/local work has been completed and pushed.
